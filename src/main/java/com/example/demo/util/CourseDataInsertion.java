...
/**
 * 课程数据插入类 - 在应用启动时自动插入Teacher1的课程数据
 */
@Component
public class CourseDataInsertion implements ApplicationRunner {

    private final CourseRepository courseRepository;

    @Autowired
    public CourseDataInsertion(CourseRepository courseRepository) {
    ...
    public void run(ApplicationArguments args) throws Exception {
        // 检查是否已经存在Teacher1的课程
        if (courseRepository.findByTeacherId("Teacher1").isEmpty()) {
            insertTeacher1Courses();
        }
    }
     ...
     * 为Teacher1插入一周的课程数据
     */
    private void insertTeacher1Courses() {
        // 教师ID
        ...
        System.out.println("成功为Teacher1插入了" + courses.size() + "门课程！");
    }
}